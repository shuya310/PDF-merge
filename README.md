...
複数のPDFを順番を決めて1つに結合するツール。処理はすべてブラウザの中で完結し、外部への通信は一切ありません。
...
| `start.command` | ローカルサーバーで開くランチャー（任意） |

## 外部通信がないことの確認方法

- `grep -n "http" pdf-merge.html pdf-merge.css` で外部URLが出ないこと（同梱ライブラリへの相対パスのみ）
- ブラウザの開発者ツール → ネットワークタブを開いた状態で結合しても、通信が発生しないこと
- インターネット接続を切った状態でも同じように動くこと
